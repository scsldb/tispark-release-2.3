...
public class RangeSplitter {
   ...
   */
  public Map<Pair<TiRegion, Metapb.Store>, ArrayList<Long>> groupByAndSortHandlesByRegionId(
      long tableId, ArrayList<Long> handles) {
    HashMap<Long, ArrayList<Long>> regionHandles = new HashMap<>();
    HashMap<Long, Pair<TiRegion, Metapb.Store>> idToRegionStorePair = new HashMap<>();
    Map<Pair<TiRegion, Metapb.Store>, ArrayList<Long>> result = new HashMap<>();
    Collections.sort(handles);

    byte[] endKey = null;
    TiRegion curRegion = null;
    ArrayList<Long> handlesInCurRegion = new ArrayList<Long>();
    for (int i = 0; i < handles.size(); i++) {
      ...
      if (endKey == null
        ...
        if (curRegion != null) {
          ...
          handlesInCurRegion = new ArrayList<Long>();
        }
    ...
    for (Map.Entry<Long, ArrayList<Long>> entry : regionHandles.entrySet()) {
      Pair<TiRegion, Metapb.Store> regionStorePair = idToRegionStorePair.get(entry.getKey());
      result.put(regionStorePair, entry.getValue());
    }
    return result;
  }

  public List<RegionTask> splitAndSortHandlesByRegion(List<Long> ids, ArrayList<Long> handles) {
   ...
   */
  private List<RegionTask> splitAndSortHandlesByRegion(long tableId, ArrayList<Long> handles) {
    // Max value for current index handle range
    ImmutableList.Builder<RegionTask> regionTasks = ImmutableList.builder();

    Map<Pair<TiRegion, Metapb.Store>, ArrayList<Long>> regionHandlesMap =
