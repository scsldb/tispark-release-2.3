...
case class ColumnarRegionTaskExec(
  ...
  def fetchTableResultsFromHandles(
    ...
    iter.flatMap { row =>
      ...
      def generateIndexTasks(handles: util.ArrayList[java.lang.Long]): util.List[RegionTask] = {
      ...
      // table scan or not.
      val newHandles = new util.ArrayList[java.lang.Long]();
      newHandles.addAll(handles.toList.map(Long.box).asJava);
      val indexTasks: util.List[RegionTask] = generateIndexTasks(newHandles)
      ...
      def feedBatch(): util.ArrayList[java.lang.Long] = {
        val handles = new util.ArrayList[java.lang.Long](512)
      ...
      def doIndexScan(): Unit =
        while (handleIterator.hasNext) {
          val handleList: util.ArrayList[java.lang.Long] = feedBatch()
